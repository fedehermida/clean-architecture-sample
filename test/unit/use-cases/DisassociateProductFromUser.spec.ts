...
describe('DisassociateProductFromUser Use Case', () => {
  let productRepo: InMemoryProductRepository;
  let userRepo: InMemoryUserRepository;
  let usecase: DisassociateProductFromUser;
  ...
  beforeEach(() => {
    ...
    usecase = new DisassociateProductFromUser(productRepo, userRepo);
  });

  describe('successful disassociation', () => {
    it('disassociates a product from a user', async () => {
      ...
      await productRepo.associateWithUser(productId, userId, product);
    ...
    it('product no longer appears in user products after disassociation', async () => {
      const user = User.create({
        id: userId,
        email: 'test@example.com',
        passwordHash: 'hashedpassword',
      ...
      expect(userProducts.length).toBe(0);
    });

    it('only removes association for specific user, not other users', async () => {
      ...
      await productRepo.save(product);
      await productRepo.associateWithUser(productId, userId, product);
      await productRepo.associateWithUser(productId, userId2, product);

      // Disassociate from user1 only
      await usecase.execute({ userId, productId });

      const user1Products = await productRepo.findByUserId(userId);
      const user2Products = await productRepo.findByUserId(userId2);

      // User1 should no longer have the product
      expect(user1Products.length).toBe(0);
      // User2 should still have the product
